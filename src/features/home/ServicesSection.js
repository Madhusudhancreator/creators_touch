...
const DEFAULT_SERVICES = [
  ...
  { number: "03", title: "Content",        tag: "Words & Design",        description: "We create clear and engaging content for your brand.",                  bullets: ["Writing","Visual Design","Motion Graphics"], accent: "pink" },
  { number: "04", title: "Website Design", tag: "Easy User Experience",  description: "We design websites and apps that look great and are easy to use.",     bullets: ["Design","User Research","Prototype"],        accent: "blue" },
  { number: "05", title: "Development",    tag: "Website Building",      description: "We build modern websites that are fast, smooth, and reliable.",        bullets: ["Frontend","CMS Setup","Performance"],        accent: "pink" },
];

function resolveAccent(token) {
  return token === "blue" ? BLUE : PINK;
}

export default function ServicesSection({ block = null }) {
  const rawItems = block?.data?.items ?? DEFAULT_SERVICES;
  /* Normalise: stored as token string "pink"/"blue", or hex already */
  const services = rawItems.map((s) => ({
    ...s,
    accent: s.accent === "pink" || s.accent === "blue" ? resolveAccent(s.accent) : s.accent,
  }));
  ...
  return (
    <section id="services" ref={sectionRef} style={{ height: `${n * 100}vh` }}>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span
                    ...
                    style={{ fontSize: pinned ? "0.78rem" : "0.9rem", color: pinned ? "#b0b7c3" : DARK }}
                  >
                    {svc.title}
                  </span>
                </div>
                {!pinned && (
                  <span className="hidden sm:inline text-[10px] uppercase tracking-[0.3em] font-medium"
              ...
              >
                ...
                <div className="hidden sm:flex flex-col justify-between w-24 lg:w-36 shrink-0 py-4">
                  <span
                    ...
                    style={{
                      ...
                      lineHeight: 1,
                      color: svc.accent === PINK ? "rgba(204,0,102,0.07)" : "rgba(9,119,168,0.07)",
                    }}
                  >
                    {svc.number}
                  </span>
                ...
                <div className="flex flex-col justify-center flex-1 min-w-0">
                  <h3
                    ...
                    style={{ fontSize: "clamp(2.2rem, 6vw, 5rem)", color: DARK }}
                  >
                    {svc.title}
                  </h3>
                  <div className="mt-5 mb-5 w-8 h-px rounded-full" style={{ backgroundColor: svc.accent }} />
                  <p className="text-sm sm:text-base leading-relaxed max-w-md" style={{ color: "#6b7280" }}>
                    {svc.description}
                  </p>
